...
- As chamadas às APIs externas (ViaCEP, WeatherAPI) usam `net/http` puro com
  ...
  em `desafios/02-multithreading` e `desafios/01-client-server-api` — sem nenhuma lib de HTTP
  client de terceiros.
- O router é o **go-chi** (`chi/v5`), o mesmo já usado no `desafios/04-rate-limiter`.
...
- Os testes seguem o mesmo estilo das aulas: tabela de casos com `testing` + `testify/assert`.
  ...
  externas (mesmo padrão de `desafios/05-stress-test`), e o handler HTTP é testado com
  `httptest.NewRequest`/`NewRecorder` (mesmo padrão de
  `desafios/04-rate-limiter/internal/httpmiddleware`), usando fakes manuais das interfaces
