...
Padrão de inversão de dependência: `httphandler` define interfaces pequenas (`CityFinder`, `TemperatureFetcher`, `ServiceBForwarder`) que os clients concretos implementam, injetadas a partir de cada `main.go` — mesmo padrão usado no desafio anterior (`desafios/06-clima-google-cloud-run`).
